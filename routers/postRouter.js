  ...
  try {
    ...
    if (dbUser !== null) {
      const posts = db.posts
        .find({ artistName: { $in: dbUser?.following } })
        .sort({ timeStamp: -1 });
      const postArray = await posts.toArray();
      let filteredArray = postArray.filter(
        (post) => post.referenceName === "wallpost"
      );
...
router.get("/api/posts/wallposts/:artistName", async (req, res) => {
  try {
    ...
    const wallPosts = await db.posts
      .find({
...
router.get("/api/posts/hyped", async (req, res) => {
  try {
    const postEligibleForHype = await db.posts
      .find({
...
router.patch(
  "/api/posts/comments/:reference/:search",
  authenticateToken,
  async (req, res) => {
    try {
      const user = req.user;
...
router.patch("/api/posts/:postid", authenticateToken, async (req, res) => {
  ...
  try {
    const checkIfUserAlreadyRatedPost = await db.posts
      .find({ _id: postId })
      .toArray();
    if (checkIfUserAlreadyRatedPost[0].rating.includes(userWhoRated)) {
      const removeUserFromRatingArray = await db.posts.updateOne(
        { _id: postId },
        { $pull: { rating: userWhoRated } }
      );
      res
        .status(200)
        .send({ length: checkIfUserAlreadyRatedPost[0]?.rating?.length - 1 });
    } else {
      const addUserToRatingArray = await db.posts.updateOne(
        { _id: postId },
        { $push: { rating: userWhoRated } }
      );
      res
        .status(200)
        .send({ length: checkIfUserAlreadyRatedPost[0]?.rating?.length + 1 });
...
router.delete("/api/posts/comments/:postid/:commentid", async (req, res) => {
  try {
    ...
    const update = db.posts.updateOne(
      { _id: postId },
