...
router.get("/api/conversations", authenticateToken, async (req, res) => {
    try{
        ...
        const mergedConversations = conversations.map((conversation) => {
            ...
            return {...conversation, profilePictureKey: user?.profilePictureKey, participantId: user?._id};
        });
...
router.patch("/api/conversations/messages/:conversationid", authenticateToken, async (req, res) => {
    try{
        ...
        const updateConversation = await db.conversations.updateOne({_id: new ObjectId(conversationId)}, {$set: {timeStamp: timeStamp, read: false, sender: user.artistName}});
