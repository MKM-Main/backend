...
router.patch(
  "/api/admin/users/:artistId/profile-picture",
  authenticateToken,
  async (req, res) => {
      ...
      if (req.user.role === "admin" || req.user._id === artistIdString) {
        if (file) {
          ...
          await s3
            .listObjectsV2({
              Bucket: "mkm-mcb",
              Prefix: `${artistIdAsObject}/profile/`,
            })
            .promise()
            .then(async (data) => {
        ...
        await db.users
          ...
          .then(async () => {
            await db.posts.updateMany(
              ...
              {
                arrayFilters: [{ "comment.artistId": artistIdString }],
              }
            );
          });
      } else {
    ...
    try {
      ...
      if (userRole === "admin" || req.user.artistName === artistName) {
        ...
        if (newArtistName) {
          ...
          const updateDocumentPromises = collectionArray.map(
            async (collectionSearch) => {
              ...
              const updateDocuments = documents.map(async (document) => {
                ...
                if (document.reported) {
                  const updatePostReported = document.reported.map(
                    async (report) => {
                      if (report.userWhoReported === artistName) {
                        await collection.updateOne(
                          ...
                          {
                            $set: {
                              "reported.$.userWhoReported": newArtistName,
                            },
                          }
                        );
                      }
                ...
                if (document.comments) {
                  const updateComments = document.comments.map(
                    async (comment) => {
                      ...
                      if (comment.reported) {
                        const updateReported = comment.reported.map(
                          async (report) => {
                            if (report.userWhoReported === artistName) {
                              ...
                              await collection.updateOne(
                                ...
                                {
                                  $set: {
                                    "comments.$[comment].reported.$[reported].userWhoReported":
                                      newArtistName,
                                  },
                                },
                                {
                      ...
                      if (
                        comment.rating &&
                        comment.rating.includes(artistName)
                      ) {
                ...
                if (document.followers) {
                  const updateFollowers = document.followers.map(
                ...
                if (document.following) {
                  const updateFollowing = document.following.map(
                    async (following, index) => {
                ...
                if (document.messages) {
                  const updateMessages = document.messages.map(
                    async (message) => {
                ...
                if (document.participants) {
                  const updateParticipants = document.participants.map(
                    async (participant) => {
                      if (participant === artistName) {
                        await collection.updateOne(
                          ...
                          {
                            $set: {
                              "participants.$[participant]": newArtistName,
                            },
                          },
        ...
        const user = req.user;
        const findUserById = await db.users
          .find({ _id: new ObjectId(user._id) })
          .toArray();
        if (req.user.artistName === artistName) {
          res.clearCookie("jwt");
          const newToken = jwt.sign(findUserById[0], jwtSecret, {
            expiresIn: "10m",
          });
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      const result = await db.forums.updateOne(
        { _id: id },
